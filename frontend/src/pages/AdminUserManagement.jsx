...
const AdminUserManagement = () => {
  ...
  const handleArchiveAccount = async (accountId) => {
    if (!window.confirm('Are you sure you want to archive this trading account?')) return
    
    setActionLoading(true)
    try {
      const response = await fetch(`${API_URL}/admin/trading-account/${accountId}/archive`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        setMessage({ type: 'success', text: 'Account archived successfully' })
        await fetchUserAccounts(selectedUser._id)
      } else {
        const data = await response.json()
        setMessage({ type: 'error', text: data.message || 'Failed to archive account' })
      }
    } catch (error) {
      setMessage({ type: 'error', text: 'Error archiving account' })
    }
    setActionLoading(false)
  }
  ...
  // Delete trading account permanently
  const handleDeleteAccount = async (accountId) => {
    if (!window.confirm('Are you sure you want to PERMANENTLY DELETE this trading account? This action cannot be undone!')) return
    
    setActionLoading(true)
    try {
      const response = await fetch(`${API_URL}/admin/trading-account/${accountId}/delete`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminId: adminUser._id })
      })
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2 text-teal-500">
                    <CreditCard size={20} />
                    <h4 className="font-semibold">Trading Accounts ({userAccounts.length})</h4>
                  </div>
                </div>
                {userAccounts.length === 0 ? (
                  ...
                  <div className="space-y-3 max-h-[60vh] overflow-y-auto">
                    {userAccounts.map(acc => (
                      <div key={acc._id} className={`bg-dark-700 rounded-lg p-4 border ${acc.status === 'Active' ? 'border-green-500/30' : acc.status === 'Archived' ? 'border-gray-600' : 'border-red-500/30'}`}>
                        {/* Header */}
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${acc.status === 'Active' ? 'bg-green-500/20' : 'bg-gray-600/20'}`}>
                              <CreditCard size={18} className={acc.status === 'Active' ? 'text-green-500' : 'text-gray-500'} />
                            </div>
                            <div>
                              <p className="text-white font-medium">{acc.accountId}</p>
                              <div className="flex items-center gap-2">
                                <span className={`text-xs px-2 py-0.5 rounded ${acc.status === 'Active' ? 'bg-green-500/20 text-green-500' : acc.status === 'Archived' ? 'bg-gray-600/20 text-gray-400' : 'bg-red-500/20 text-red-500'}`}>
                                  {acc.status}
                                </span>
                          ...
                          <div className="text-right">
                            <p className="text-white text-xl font-bold">${acc.balance?.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) || '0.00'}</p>
                            <p className="text-gray-500 text-xs">Balance</p>
                          </div>
                        </div>
                        
                        {/* Account Details Grid */}
                        <div className="grid grid-cols-4 gap-2 mb-3 p-3 bg-dark-800 rounded-lg">
                          <div className="text-center">
                            <p className="text-gray-500 text-xs">Leverage</p>
                            <p className="text-white text-sm font-medium">{acc.leverage || '1:100'}</p>
                          </div>
                          <div className="text-center">
                            <p className="text-gray-500 text-xs">Credit</p>
                            <p className="text-purple-400 text-sm font-medium">${acc.credit?.toFixed(2) || '0.00'}</p>
                          </div>
                          <div className="text-center">
                            <p className="text-gray-500 text-xs">Equity</p>
                            <p className="text-white text-sm font-medium">${((acc.balance || 0) + (acc.credit || 0)).toFixed(2)}</p>
                          </div>
                          <div className="text-center">
                            <p className="text-gray-500 text-xs">Min Deposit</p>
                            <p className="text-yellow-400 text-sm font-medium">${acc.accountTypeId?.minDeposit || 0}</p>
                          </div>
                        </div>
                        
                        {/* Additional Info */}
                        <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                          <div className="flex justify-between p-2 bg-dark-800 rounded">
                            <span className="text-gray-500">Created:</span>
                            <span className="text-gray-300">{new Date(acc.createdAt).toLocaleDateString()}</span>
                          </div>
                          <div className="flex justify-between p-2 bg-dark-800 rounded">
                            <span className="text-gray-500">Account ID:</span>
                            <span className="text-gray-300">{acc._id?.slice(-8)}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-2 flex gap-2">
                          {acc.status === 'Archived' ? (
                            <button 
                              ...
                              disabled={actionLoading}
                              className="flex-1 py-2 text-xs bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500/30 transition-colors flex items-center justify-center gap-1"
                            >
                              <RefreshCw size={14} /> Restore Account
                            </button>
                          ) : (
                            <button 
                              onClick={() => handleArchiveAccount(acc._id)}
                              disabled={actionLoading}
                              className="flex-1 py-2 text-xs bg-gray-500/20 text-gray-400 rounded-lg hover:bg-gray-500/30 transition-colors flex items-center justify-center gap-1"
                            >
                              <Trash2 size={14} /> Archive Account
                            </button>
                          )}
                          <button 
                            onClick={() => handleDeleteAccount(acc._id)}
                            disabled={actionLoading}
                            className="flex-1 py-2 text-xs bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors flex items-center justify-center gap-1"
                          >
