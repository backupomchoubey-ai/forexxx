...
async function getFreshPrice(symbol) {
  try {
    ...
    if (price) {
      // Use price field as fallback when bid/ask are 0 (market closed)
      const lastPrice = price.price || 0
      const bid = price.bid > 0 ? price.bid : lastPrice
      const ask = price.ask > 0 ? price.ask : lastPrice
      
      return { bid, ask, price: lastPrice }
    }
...
// POST /api/trade/partial-close - MT5-style partial close a trade
router.post('/partial-close', async (req, res) => {
  try {
    const { tradeId, closeQuantity, bid, ask } = req.body

    if (!tradeId || !closeQuantity) {
      return res.status(400).json({ 
        success: false, 
        message: 'Trade ID and close quantity are required' 
      })
    }

    if (!bid || !ask || parseFloat(bid) <= 0 || parseFloat(ask) <= 0) {
      return res.status(400).json({ 
        success: false, 
        message: 'Market is closed or no price data available.',
        code: 'MARKET_CLOSED'
      })
    }

    const result = await tradeEngine.partialCloseTrade(
      tradeId,
      parseFloat(closeQuantity),
      parseFloat(bid),
      parseFloat(ask),
      'USER'
    )
    ...
    res.json({
      ...
      originalTrade: result.originalTrade,
      partialTrade: result.partialTrade,
      realizedPnl: result.realizedPnl,
      remainingQuantity: result.remainingQuantity
    })
  } catch (error) {
    console.error('Error partial closing trade:', error)
    res.status(400).json({ 
      success: false, 
      message: error.message 
    })
...
// PUT /api/trade/trailing-stop - Set/update trailing stop for a trade
router.put('/trailing-stop', async (req, res) => {
  try {
    const { tradeId, trailingStop } = req.body
    ...
    const trade = await tradeEngine.updateTrailingStop(
      tradeId,
    ...
    res.json({
      success: true,
      message: trailingStop > 0 ? 'Trailing stop activated' : 'Trailing stop deactivated',
      trade
    })
  } catch (error) {
