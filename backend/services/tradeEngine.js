...
class TradeEngine {
  ...
  calculatePnl(side, openPrice, currentPrice, quantity, contractSize = this.CONTRACT_SIZE) {
    ...
    // Sanity check: PnL should not be more than 100x the trade value
    const tradeValue = quantity * contractSize * openPrice
    const maxReasonablePnl = tradeValue * 100 // Max 100x trade value
    
    if (Math.abs(rawPnl) > maxReasonablePnl) {
      console.error('Unrealistic PnL calculation detected:', {
        side,
        openPrice,
        currentPrice,
        quantity,
      ...
      return 0 // Return 0 for unrealistic PnL
    }
  ...
  // Validate if trade can be opened (MT5-style with floating PnL in equity calculation)
  async validateTradeOpen(tradingAccountId, symbol, side, quantity, openPrice, leverage, contractSize = this.CONTRACT_SIZE, currentPrices = null) {
    ...
    // MT5-STYLE: Equity = Balance + Credit + Floating PnL (real-time)
    let floatingPnl = 0
    if (currentPrices && openTrades.length > 0) {
      for (const trade of openTrades) {
        const prices = currentPrices[trade.symbol]
        if (prices) {
          floatingPnl += this.calculateFloatingPnl(trade, prices.bid, prices.ask)
    ...
    // Free margin = Equity - Used Margin (MT5 formula)
    const freeMargin = equity - usedMargin
    ...
    return { valid: true, marginRequired, freeMargin, usedMargin, equity, floatingPnl }
  }
  ...
  async checkStopOut(tradingAccountId, currentPrices) {
    ...
    if (shouldStopOut) {
      ...
      // MT5-STYLE: Force close trades starting from the largest margin first
      const tradesWithMargin = openTrades.map(trade => {
        const prices = currentPrices[trade.symbol]
        const pnl = prices ? this.calculateFloatingPnl(trade, prices.bid, prices.ask) : 0
        return { trade, pnl, prices, margin: trade.marginUsed || 0 }
      }).sort((a, b) => b.margin - a.margin) // Sort by margin descending (largest margin first - MT5 style)

      const closedTrades = []
      for (const { trade, prices } of tradesWithMargin) {
        if (prices) {
          try {
  ...
  // MT5-STYLE: Triple swap on Wednesday (covers weekend)
  async applySwap() {
    ...
    // MT5: Check if today is Wednesday (triple swap day)
    const now = new Date()
    const dayOfWeek = now.getUTCDay() // 0 = Sunday, 3 = Wednesday
    const isTripleSwapDay = dayOfWeek === 3 // Wednesday
    const swapMultiplier = isTripleSwapDay ? 3 : 1

    console.log(`[SWAP] Applying swap with multiplier: ${swapMultiplier} (Wednesday: ${isTripleSwapDay})`)
    ...
    for (const trade of openTrades) {
      ...
      // Apply triple swap on Wednesday (MT5 style)
      swapAmount *= swapMultiplier

      trade.swap += swapAmount
      await trade.save()
      
      console.log(`[SWAP] Trade ${trade.tradeId}: ${swapAmount.toFixed(4)} (rate: ${swapRate}, multiplier: ${swapMultiplier})`)
    }
  }

  // MT5-STYLE: Partial close a trade
  async partialCloseTrade(tradeId, closeQuantity, currentBid, currentAsk, closedBy = 'USER') {
    const trade = await Trade.findById(tradeId).populate({ path: 'tradingAccountId', populate: { path: 'accountTypeId' } })
    if (!trade) throw new Error('Trade not found')
    if (trade.status !== 'OPEN') throw new Error('Trade is not open')
    
    // Validate close quantity
    if (closeQuantity <= 0) throw new Error('Close quantity must be positive')
    if (closeQuantity >= trade.quantity) throw new Error('Use full close for closing entire position')
    if (closeQuantity > trade.quantity) throw new Error('Close quantity exceeds position size')

    const closePrice = trade.side === 'BUY' ? currentBid : currentAsk
    
    // Calculate PnL for the partial close
    const partialPnl = this.calculatePnl(trade.side, trade.openPrice, closePrice, closeQuantity, trade.contractSize)
    
    // Calculate proportional commission and swap
    const ratio = closeQuantity / trade.quantity
    const partialCommission = trade.commission * ratio
    const partialSwap = trade.swap * ratio
    const realizedPnl = partialPnl - partialSwap
    
    // Update original trade (reduce quantity)
    const remainingQuantity = trade.quantity - closeQuantity
    const remainingMargin = this.calculateMargin(remainingQuantity, trade.openPrice, `1:${trade.leverage}`, trade.contractSize)
    
    trade.quantity = remainingQuantity
    trade.marginUsed = remainingMargin
    trade.commission = trade.commission - partialCommission
    trade.swap = trade.swap - partialSwap
    await trade.save()
    
    // Create a closed trade record for the partial close
    const partialTradeId = await Trade.generateTradeId()
    const partialTrade = await Trade.create({
      userId: trade.userId,
    ...
    console.log(`[PARTIAL CLOSE] Trade ${trade.tradeId}: Closed ${closeQuantity} lots at ${closePrice}, PnL: $${realizedPnl.toFixed(2)}, Remaining: ${remainingQuantity} lots`)
    
    return {
      originalTrade: trade,
      partialTrade: partialTrade,
      realizedPnl,
      remainingQuantity
    }
  }

  // MT5-STYLE: Update trailing stop for a trade
  async updateTrailingStop(tradeId, trailingStopDistance) {
    const trade = await Trade.findById(tradeId)
    if (!trade) throw new Error('Trade not found')
    if (trade.status !== 'OPEN') throw new Error('Trade is not open')
    
    trade.trailingStop = trailingStopDistance
    trade.trailingStopActive = trailingStopDistance > 0
    await trade.save()
    
    return trade
  }

  // MT5-STYLE: Check and update trailing stops for all trades
  async checkTrailingStops(currentPrices) {
    ...
    for (const trade of openTrades) {
      ...
      const trailingDistance = trade.trailingStop
      
      // Calculate pip value based on symbol
      let pipValue = 0.0001 // Default for forex
      if (trade.symbol.includes('JPY')) pipValue = 0.01
      if (['XAUUSD', 'XAGUSD'].includes(trade.symbol)) pipValue = 0.01
      
      const trailingDistancePrice = trailingDistance * pipValue
      
      if (trade.side === 'BUY') {
        // For BUY: Move SL up when price moves up
        const newSL = currentPrice - trailingDistancePrice
        const currentSL = trade.stopLoss || trade.sl || 0
        
        // Only move SL if new SL is higher than current SL and price is in profit
        if (newSL > currentSL && newSL > trade.openPrice) {
          trade.stopLoss = newSL
          trade.sl = newSL
          await trade.save()
          updatedTrades.push({ trade, newSL, reason: 'TRAILING_STOP_MOVED' })
          console.log(`[TRAILING STOP] Trade ${trade.tradeId}: SL moved to ${newSL.toFixed(5)}`)
        }
      } else {
        // For SELL: Move SL down when price moves down
        const newSL = currentPrice + trailingDistancePrice
        const currentSL = trade.stopLoss || trade.sl || Infinity
        
        // Only move SL if new SL is lower than current SL and price is in profit
        if (newSL < currentSL && newSL < trade.openPrice) {
          trade.stopLoss = newSL
          trade.sl = newSL
          await trade.save()
